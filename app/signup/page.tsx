...
export default function SignupPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(err?.code ? friendlyAuthError(err.code) : err?.message || 'Signup failed.');
    }
    setBusy(null);
  }
    ...
    <div className="px-6 py-16 max-w-md mx-auto">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        ...
        <p className="text-sm text-concrete-muted mb-8">
          One account for orders, store credit and faster checkout.
        </p>
        ...
        <p className="text-sm text-concrete-muted text-center">
          ...
          <Link href="/login" className="text-brass hover:text-obsidian transition-colors font-medium">
            Sign in
          </Link>
        </p>
      </motion.div>
